...
def render_launcher(size: int, *, bg=CREAM, ink=INK, fill_frac: float = 0.45) -> Image.Image:
...
def regen_icon_foreground():
    ...
    for d in DENSITIES:
        ...
        img = render_transparent(size, size, INK, fill_frac=0.29)
        for n in names:
...
def regen_logo_middle():
    ...
    for d, size in (("xhdpi", 136), ("xxhdpi", 204)):
        ...
        if os.path.exists(path):
            save(render_launcher(size, fill_frac=0.45), path)
            written += 1
    return written
